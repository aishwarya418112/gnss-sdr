...
#include "navic_l5_ca_dll_pll_tracking.h"
...
std::unique_ptr<GNSSBlockInterface> GNSSBlockFactory::GetObservables(const ConfigurationInterface* configuration)
{
    ...
    unsigned int NAVIC_channels = configuration->property("Channels_L5.count", 0);
    //NAVIC_channels += configuration->property("Channels_S.count", 0);
    ...
    return GetBlock(configuration, "Observables",
        Galileo_channels +
            GPS_channels +
            Glonass_channels +
            Beidou_channels + NAVIC_channels + 
            extra_channels,
        Galileo_channels +
            GPS_channels +
            Glonass_channels +
    ...
    Concurrent_Queue<pmt::pmt_t>* queue)
{
    ...
    const unsigned int Channels_S_count = configuration->property("Channels_S.count", 0);
    ...
    try
        {
            ...
            // **************** NAVIC L5 CHANNELS ********************************
            LOG(INFO) << "Getting " << Channels_L5_count << " NAVIC L5 channels";
            ...
            // **************** NAVIC S CHANNELS ********************************
            LOG(INFO) << "Getting " << Channels_S_count << " NAVIC S channels";

            for (unsigned int i = 0; i < Channels_S_count; i++)
                {
                    // Store the channel into the vector of channels
                    channels->at(channel_absolute_id) = GetChannel(configuration,
                        std::string("S"),
    ...
    Concurrent_Queue<pmt::pmt_t>* queue)
{
    ...
    // ACQUISITION BLOCKS ------------------------------------------------------
    else if (implementation == "GPS_L1_CA_PCPS_Acquisition_GPS")
        {
            std::unique_ptr<GNSSBlockInterface> block_ = std::make_unique<GpsL1CaPcpsAcquisitionGPS>(configuration, role, in_streams,
                out_streams);
            block = std::move(block_);
        }
    else if (implementation == "NAVIC_L5_CA_PCPS_Acquisition")
        {
            std::unique_ptr<GNSSBlockInterface> block_ = std::make_unique<NavicL5CaPcpsAcquisition>(configuration, role, in_streams,
                out_streams);
            block = std::move(block_);
        }
    ...
    else if (implementation == "NAVIC_L5_CA_DLL_PLL_Tracking")
        {
            std::unique_ptr<GNSSBlockInterface> block_ = std::make_unique<NavicL5CaDllPllTracking>(configuration, role, in_streams,
                out_streams);
            block = std::move(block_);
        }
    ...
    unsigned int out_streams)
{
    ...
    // ACQUISITION BLOCKS ------------------------------------------------------
    if (implementation == "GPS_L1_CA_PCPS_Acquisition_GPS")
        {
            std::unique_ptr<AcquisitionInterface> block_ = std::make_unique<GpsL1CaPcpsAcquisitionGPS>(configuration, role, in_streams,
                out_streams);
            block = std::move(block_);
        }
    else if (implementation == "NAVIC_L5_CA_PCPS_Acquisition")
        {
            std::unique_ptr<AcquisitionInterface> block_ = std::make_unique<NavicL5CaPcpsAcquisition>(configuration, role, in_streams,
                out_streams);
            block = std::move(block_);
        }
    else if (implementation == "GPS_L1_CA_PCPS_Assisted_Acquisition")
    ...
    unsigned int out_streams)
{
    ...
    if (implementation == "GPS_L1_CA_DLL_PLL_Tracking")
        ...
        {
            std::unique_ptr<TrackingInterface> block_ = std::make_unique<NavicL5CaDllPllTracking>(configuration, role, in_streams,
                out_streams);
            block = std::move(block_);
        }
    else if (implementation == "GPS_L1_CA_KF_Tracking")
