/*!
 * \file navic_ephemeris.cc
 * \brief  Interface of a Navic EPHEMERIS storage and orbital model functions
...
double Navic_Ephemeris::Navic_System_Time(double WN, double TOW)
{
    /* NAVIC SYSTEM TIME, ICD 5.1.2
     * input parameter:
     * WN: The Week Number is an integer counter that gives the sequential week number
       from the origin of the Navic time. It covers 4096 weeks (about 78 years).
       ...
       output:
       t: it is the transmitted time in Navic System Time (expressed in seconds)
       ...
       message provided through the TOW is synchronised to each satellite’s version of Navic System Time (GST).
     *
     */
    const double sec_in_day = 86400;
    const double day_in_week = 7;
    double t = WN * sec_in_day * day_in_week + TOW;  // second from the origin of the Navic time
    return t;
}


double Navic_Ephemeris::sv_clock_drift(double transmitTime)
{
    // Satellite Time Correction Algorithm, ICD 5.1.4
    const double dt = transmitTime - t0c_4;
    Navic_satClkDrift = af0_4 + af1_4 * dt + af2_4 * (dt * dt) + sv_clock_relativistic_term(transmitTime);  // +Navic_dtr;
    return Navic_satClkDrift;
}


// compute the relativistic correction term
double Navic_Ephemeris::sv_clock_relativistic_term(double transmitTime)  // Satellite Time Correction Algorithm, ICD 5.1.4
{
    // Restore semi-major axis
    const double a = A_1 * A_1;

    const double n0 = sqrt(NAVIC_GM / (a * a * a));

    // Time from ephemeris reference epoch
    // t = WN_5*86400*7 + TOW_5; //WN_5*86400*7 are the second from the origin of the Navic time
    ...
    Navic_dtr = NAVIC_F * e_1 * A_1 * sin(E);
    return Navic_dtr;
}


void Navic_Ephemeris::satellitePosition(double transmitTime)
{
    // when this function in used, the input must be the transmitted time (t) in
    // seconds computed by Navic_System_Time (above function)

    // Find Navic satellite's position ---------------------------------------
    ...
    d_satpos_Y = xprime * sok + yprime * cik * cok;  // ********NOTE: in NAVIC ICD this expression is not correct because it has minus (- sin(u) * r * cos(i) * cos(Omega)) instead of plus
...
double Navic_Ephemeris::check_t(double time)
{
