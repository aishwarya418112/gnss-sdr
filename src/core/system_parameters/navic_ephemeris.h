/*!
 * \file navic_ephemeris.h
 * \brief  Interface of a Navic EPHEMERIS storage
...
#ifndef GNSS_SDR_NAVIC_EPHEMERIS_H
#define GNSS_SDR_NAVIC_EPHEMERIS_H
...
/*!
 * \brief This class is a storage and orbital model functions for the Navic SV ephemeris data as described in Navic ICD paragraph 5.1.1
 *  (See https://www.gsc-europa.eu/sites/default/files/sites/all/files/Navic-OS-SIS-ICD.pdf )
 *
 */
class Navic_Ephemeris
{
public:
    Navic_Ephemeris() = default;

    void satellitePosition(double transmitTime);             //!< Computes the ECEF SV coordinates and ECEF velocity
    double Navic_System_Time(double WN, double TOW);       //!< Navic System Time (GST), ICD paragraph 5.1.2
    ...
    /* Navic ephemeris are 16 parameters and here are reported following the ICD order, paragraph 5.1.1.
    ...
    double Navic_dtr{};  //!< relativistic clock correction term
    ...
    inline void serialize(Archive& archive, const uint32_t version)
    {
        ...
        archive& BOOST_SERIALIZATION_NVP(Navic_satClkDrift);
        archive& BOOST_SERIALIZATION_NVP(Navic_dtr);
        ...
        archive& BOOST_SERIALIZATION_NVP(L5_HS_5);
  
        archive& BOOST_SERIALIZATION_NVP(L5_DVS_5);
...
/** \} */
/** \} */
#endif  // GNSS_SDR_NAVIC_EPHEMERIS_H
