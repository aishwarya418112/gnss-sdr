...
using CRC_Navic_NAV_type = boost::crc_optimal<24, 0x1864CFBU, 0x0, 0x0, false, false>;


bool Navic_nav_Message::CRC_test(std::bitset<NAVIC_DATA_FRAME_BITS> bits, uint32_t checksum) const
{
    CRC_Navic_NAV_type CRC_Navic;

    // Navic INAV frame for CRC is not an integer multiple of bytes
    ...
    CRC_Navic.process_bytes(bytes.data(), NAVIC_DATA_FRAME_BYTES);

    const uint32_t crc_computed = CRC_Navic.checksum();
...
uint64_t Navic_nav_Message::read_navigation_unsigned(std::bitset<NAVIC_DATA_JK_BITS> bits, const std::vector<std::pair<int32_t, int32_t> >& parameter) const
{
    uint64_t value = 0ULL;
    ...
    for (int32_t i = 0; i < num_of_slices; i++)
        {
            for (int32_t j = 0; j < parameter[i].second; j++)
                {
                    value <<= 1U;  // shift left
                    if (static_cast<int>(bits[NAVIC_DATA_JK_BITS - parameter[i].first - j]) == 1)
                        {
...
uint64_t Navic_nav_Message::read_page_type_unsigned(std::bitset<NAVIC_PAGE_TYPE_BITS> bits, const std::vector<std::pair<int32_t, int32_t> >& parameter) const
{
    uint64_t value = 0ULL;
    const int32_t num_of_slices = parameter.size();
    for (int32_t i = 0; i < num_of_slices; i++)
        {
            for (int32_t j = 0; j < parameter[i].second; j++)
                {
                    value <<= 1U;  // shift left
                    if (static_cast<int>(bits[NAVIC_PAGE_TYPE_BITS - parameter[i].first - j]) == 1)
                        {
...
int64_t Navic_nav_Message::read_navigation_signed(std::bitset<NAVIC_DATA_JK_BITS> bits, const std::vector<std::pair<int32_t, int32_t> >& parameter) const
{
    int64_t value = 0LL;
    const int32_t num_of_slices = parameter.size();

    // read the MSB and perform the sign extension
    if (static_cast<int>(bits[NAVIC_DATA_JK_BITS - parameter[0].first]) == 1)
        {
...
bool Navic_nav_Message::read_navigation_bool(std::bitset<NAVIC_DATA_JK_BITS> bits, const std::vector<std::pair<int32_t, int32_t> >& parameter) const
{
    bool value;
    if (static_cast<int>(static_cast<int>(bits[NAVIC_DATA_JK_BITS - parameter[0].first])) == 1)
        {
            value = true;
        }
    else
...
void Navic_nav_Message::split_page(std::string page_string, int32_t flag_even_word)
{
    int32_t Page_type = 0;
    ...
    if (page_string.at(0) == '1')  // if page is odd
        {
            ...
            if (flag_even_word == 1)  // An odd page has been received but the previous even page is kept in memory and it is considered to join pages
                {
                    const std::string page_NAV_even = page_Even;
                    const std::string page_NAV = page_NAV_even + page_Odd;  // Join pages: Even + Odd = INAV page
                    const std::string Even_bit = page_NAV.substr(0, 1);
                    const std::string Page_type_even = page_NAV.substr(1, 1);
                    const std::string nominal = "0";

                    const std::string Data_k = page_NAV.substr(2, 112);
                    const std::string Odd_bit = page_NAV.substr(114, 1);
                    const std::string Page_type_Odd = page_NAV.substr(115, 1);
                    const std::string Data_j = page_NAV.substr(116, 16);

                    const std::string Reserved_1 = page_NAV.substr(132, 40);
                    const std::string SAR = page_NAV.substr(172, 22);
                    const std::string Spare = page_NAV.substr(194, 2);
                    const std::string CRC_data = page_NAV.substr(196, 24);
                    const std::string Reserved_2 = page_NAV.substr(220, 8);
                    const std::string Tail_odd = page_NAV.substr(228, 6);

                    // ************ CRC checksum control *******/
                    std::stringstream TLM_word_for_CRC_stream;
                    TLM_word_for_CRC_stream << page_INAV;
                    const std::string TLM_word_for_CRC = TLM_word_for_CRC_stream.str().substr(0, NAVIC_DATA_FRAME_BITS);
                    const std::bitset<NAVIC_DATA_FRAME_BITS> TLM_word_for_CRC_bits(TLM_word_for_CRC);
                    ...
                    if (CRC_test(TLM_word_for_CRC_bits, checksum.to_ulong()) == true)
                        {
                            ...
                            const std::bitset<NAVIC_PAGE_TYPE_BITS> page_type_bits(page_number_bits);  // from string to bitset
...
bool Navic_Inav_Message::have_new_ephemeris()  // Check if we have a new ephemeris stored in the navic navigation class
{
...
bool Navic_nav_Message::have_new_iono_and_GST()  // Check if we have a new iono data set stored in the navic navigation class
{
...
bool Navic_nav_Message::have_new_utc_model()  // Check if we have a new utc data set stored in the navic navigation class
{
...
bool Navic_nav_Message::have_new_almanac()  // Check if we have a new almanac data set stored in the navic navigation class
{
...
Navic_Ephemeris Navic_nav_Message::get_ephemeris() const
{
    Navic_Ephemeris ephemeris;
    ...
    ephemeris.L5_HS_5 = L5_HS_5;    // E1B Signal Health Status
    
    ephemeris.L5_DVS_5 = L5_DVS_5;  // E1B Data Validity Status
    ...
    ephemeris.Navic_satClkDrift = Navic_satClkDrift;

    return ephemeris;
}


Navic_Iono Navic_nav_Message::get_iono() const
{
    Navic_Iono iono;
...
Navic_Utc_Model Navic_nav_Message::get_utc_model() const
{
    Navic_Utc_Model utc_model;
    ...
    // GPS to Navic GST conversion parameters
...
Navic_Almanac_Helper Navic_nav_Message::get_almanac() const
{
    Navic_Almanac_Helper almanac;
    ...
    almanac.L5_HS_10 = L5_HS_10;

    return almanac;
}


int32_t Navic_nav_Message::page_jk_decoder(const char* data_jk)
{
    const std::string data_jk_string = data_jk;
    const std::bitset<NAVIC_DATA_JK_BITS> data_jk_bits(data_jk_string);
    ...
    switch (page_number)
        ...
        case 5:  // Word type 5: Ionospheric correction, BGD, signal health and data validity status and GST
            ...
            L5_HS_5 = static_cast<int32_t>(read_navigation_unsigned(data_jk_bits, L5_HS_5_BIT));
            DLOG(INFO) << "L5_HS_5= " << L5_HS_5;
            
            L5_DVS_5 = static_cast<bool>(read_navigation_unsigned(data_jk_bits, L5_DVS_5_BIT));
            DLOG(INFO) << "L5_DVS_5= " << L5_DVS_5;
            // GST
        ...
        case 10:  // Word type 10: Almanac for SVID3 (2/2) and GST-GPS conversion parameters
            ...
            L5_HS_10 = static_cast<int32_t>(read_navigation_unsigned(data_jk_bits, L5_HS_10_BIT));
            DLOG(INFO) << "L5_HS_10= " << L5_HS_10;
