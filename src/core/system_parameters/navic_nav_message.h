/*!
 * \file navic_nav_message.h
 * \brief  Implementation of a Navic NAV Data message
 *         as described in Navic OS SIS ICD Issue 1.2 (Nov. 2015)
...
#ifndef GNSS_SDR_NAVIC_NAV_MESSAGE_H
#define GNSS_SDR_NAVIC_NAV_MESSAGE_H

#include "Navic_NAV.h"
#include "navic_almanac_helper.h"
#include "navic_ephemeris.h"
#include "navic_iono.h"
#include "navic_utc_model.h"
#include <bitset>
#include <cstdint>
#include <string>
#include <utility>
...
/*!
 * \brief This class handles the Navic I/NAV Data message, as described in the
 * Navic Open Service Signal in Space Interface Control Document (OS SIS ICD), Issue 1.2 (Nov 2015).
 * See https://www.gsc-europa.eu/sites/default/files/sites/all/files/Navic-OS-SIS-ICD.pdf
 */
class Navic_Inav_Message
{
public:
    Navic_Inav_Message() = default;
    ...
    /*
     * \brief Returns a Navic_Ephemeris object filled with the latest navigation data received
     */
    Navic_Ephemeris get_ephemeris() const;

    /*
     * \brief Returns a Navic_Iono object filled with the latest navigation data received
     */
    Navic_Iono get_iono() const;

    /*
     * \brief Returns a Navic_Utc_Model object filled with the latest navigation data received
     */
    Navic_Utc_Model get_utc_model() const;

    /*
     * \brief Returns a Navic_Almanac_Helper object filled with the latest navigation data received
     */
    Navic_Almanac_Helper get_almanac() const;
    ...
    inline int32_t get_Navic_week() const
    {
        return WN_0;
    }
...
private:
    bool CRC_test(std::bitset<NAVIC_DATA_FRAME_BITS> bits, uint32_t checksum) const;
    bool read_navigation_bool(std::bitset<NAVIC_DATA_JK_BITS> bits, const std::vector<std::pair<int32_t, int32_t> >& parameter) const;
    uint64_t read_navigation_unsigned(std::bitset<NAVIC_DATA_JK_BITS> bits, const std::vector<std::pair<int32_t, int32_t> >& parameter) const;
    uint64_t read_page_type_unsigned(std::bitset<NAVIC_PAGE_TYPE_BITS> bits, const std::vector<std::pair<int32_t, int32_t> >& parameter) const;
    int64_t read_navigation_signed(std::bitset<NAVIC_DATA_JK_BITS> bits, const std::vector<std::pair<int32_t, int32_t> >& parameter) const;
    ...
    int32_t L5_HS_5{};  // L5 Signal Health Status
    
    bool L5_DVS_5{};    // L5 Data Validity Status
    ...
    int32_t t_0G_10{};   // Reference time for Navic/GPS Time Offset (GGTO) data
    int32_t WN_0G_10{};  // Week Number of Navic/GPS Time Offset (GGTO) reference
...
/** \} */
/** \} */
#endif  // GNSS_SDR_NAVIC_NAV_MESSAGE_H
